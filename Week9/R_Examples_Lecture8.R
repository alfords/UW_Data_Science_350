...
# Create a prior:  here it will be a shifted/reflected absolute value shape = /\
...
# New CI
mean(boot_means) + 1.98 * mean(boot_sds)
mean(boot_means) - 1.98 * mean(boot_sds)

# Use the boot function
mean_fun = function(data, ix) mean(data[ix], na.rm = TRUE) # Boot function requires
...
boot_coef = lapply(point_indices, function(x){
  temp_lm = lm(admit_rate[x] ~ hospitalizations[x])
